...
import { GitBranch, Layout, Maximize2, ChevronLeft, ChevronRight } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
  ...
  const renderCard = (item: any, i: number) => {
    return (
      <Dialog key={i}>
        <DialogTrigger asChild>
          <button className="relative group overflow-hidden bg-white rounded-3xl p-4 flex flex-col items-center gap-2 shadow-lg hover:shadow-xl transition-all w-full border border-slate-50">
            <div className="absolute inset-0 bg-[#FF1E2D]/95 opacity-0 group-hover:opacity-100 transition-opacity z-10 flex flex-col items-center justify-center p-4">
              ...
              <span className="text-white font-black text-[10px] uppercase tracking-widest italic">AMPLIAR</span>
            </div>
        ...
        <DialogContent className="max-w-[95vw] w-full h-[90vh] p-0 overflow-hidden flex flex-col bg-white border-none shadow-2xl z-[400]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-4 border-b-2 border-slate-100 pb-2">
            ...
            <h3 className="text-lg md:text-2xl font-black uppercase italic text-[#2B2B2B]">MODELADO UML</h3>
          </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-4 border-b-2 border-slate-100 pb-2">
            ...
            <h3 className="text-lg md:text-2xl font-black uppercase italic text-[#2B2B2B]">PROTOTIPOS UX/UI</h3>
          </div>
          
          <Dialog>
            <DialogTrigger asChild>
              <button className="relative group w-full h-[300px] md:h-full max-h-[400px] bg-slate-50 rounded-[30px] overflow-hidden border-2 border-dashed border-slate-200 shadow-lg transition-all hover:border-[#FF1E2D]/50">
            ...
            <DialogContent className="max-w-[95vw] w-full h-[90vh] p-0 overflow-hidden flex flex-col bg-white border-none z-[400]">
              <DialogHeader className="p-4 bg-[#2B2B2B] text-white border-b-4 border-[#FF1E2D]">
                <DialogTitle className="text-xl md:text-3xl font-black uppercase italic text-center">DISEÑO DE INTERFACES SIBF-CAI</DialogTitle>
              </DialogHeader>
                  ...
                  <CarouselContent>
                    {uxImages.map((img, i) => (
                      <CarouselItem key={i} className="flex justify-center items-center">
                        <div className="relative group rounded-[40px] overflow-hidden shadow-2xl border-4 border-white bg-white aspect-[9/16] h-[70vh]">
                          <img src={img} className="w-full h-full object-cover" alt={`UI Interface ${i + 1}`} />
                        </div>
                      </CarouselItem>
                    ))}
                  </CarouselContent>
                  <CarouselPrevious className="left-[-20px] md:left-[-60px] h-12 w-12 bg-white text-[#FF1E2D] hover:bg-[#FF1E2D] hover:text-white" />
                  <CarouselNext className="right-[-20px] md:right-[-60px] h-12 w-12 bg-white text-[#FF1E2D] hover:bg-[#FF1E2D] hover:text-white" />
                </Carousel>
              </div>
            </DialogContent>
          </Dialog>
