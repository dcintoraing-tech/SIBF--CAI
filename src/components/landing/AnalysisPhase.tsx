...
export default function AnalysisPhase() {
  return (
    <div className="w-full h-full flex flex-col justify-center space-y-4 md:space-y-10 max-w-6xl mx-auto">
      ...
      <p className="text-sm md:text-3xl lg:text-4xl text-[#2B2B2B] font-bold leading-[1.1] max-w-5xl uppercase italic border-l-4 md:border-l-8 border-[#FF1E2D] pl-4 md:pl-6">
        Análisis técnico mediante <span className="text-[#FF1E2D]">ENTREVISTAS DIRECTAS</span> y auditoría de procesos institucionales.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
        <Card className="rounded-xl border-none bg-white shadow-xl p-4 md:p-8 hover:bg-[#FF1E2D]/5 transition-colors">
          ...
          <CardContent className="p-0">
            <p className="text-[10px] md:text-xl text-[#2B2B2B] font-bold uppercase leading-tight italic">
          ...
          <CardContent className="p-0 space-y-2 md:space-y-4">
            {["SEGURIDAD BIOMÉTRICA", "RESPUESTA < 2S", "99.9% DISPONIBILIDAD"].map((item, i) => (
              <div key={i} className="flex justify-between items-center border-b border-gray-100 pb-2">
                <span className="text-[8px] md:text-base lg:text-xl font-black text-[#2B2B2B] uppercase tracking-widest">{item}</span>
                <span className="text-[8px] md:text-base lg:text-xl font-black text-[#FF1E2D] italic">✓ OK</span>
              </div>
            ))}
          </CardContent>
        </Card>
